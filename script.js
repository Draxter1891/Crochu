...
const PRODUCTS = [
  {
    ...
    reviews: 132,
    tag: "Bestseller",
    img: IMG("photo-1584992236310-6edddc08acff"),
    seed: "blankey",
  },
  {
    id: 2,
    name: "Sunny the Sunflower Plushie",
    cat: "Plushies",
    price: 22,
    rating: 4.8,
    reviews: 96,
    tag: "New",
    img: IMG("photo-1615486511484-92e172cc4fe0"),
    seed: "sunflower",
  },
  {
    id: 3,
    name: "Lavender Dreams Coaster Set",
    cat: "Home",
    price: 16,
    rating: 4.7,
    reviews: 58,
  ...
  {
    id: 4,
    name: "Strawberry Kiss Beanie",
    cat: "Wearables",
    price: 28,
    rating: 4.8,
    reviews: 74,
    tag: "",
    img: IMG("photo-1576871337622-98d48d1cf531"),
    seed: "beanie",
  },
  {
    id: 5,
    name: "Boo the Bunny Amigurumi",
    cat: "Plushies",
    price: 26,
    rating: 5.0,
    reviews: 141,
    tag: "Bestseller",
    img: IMG("photo-1615484477778-ca3b77940c25"),
    seed: "bunny",
  },
  {
    id: 6,
    name: "Mint Wave Market Tote",
    cat: "Bags",
    price: 34,
    rating: 4.6,
    reviews: 63,
    tag: "",
    img: IMG("photo-1591561954557-26941169b49e"),
    seed: "tote",
  },
  {
    id: 7,
    name: "Rainbow Dreams Nursery Mobile",
    cat: "Nursery",
    price: 52,
    rating: 4.9,
    reviews: 87,
    tag: "",
    img: IMG("photo-1519689680058-324335c77eba"),
    seed: "mobile",
  },
  {
    id: 8,
    name: "Daisy Granny Square Cardigan",
    cat: "Wearables",
    price: 68,
    rating: 4.8,
    reviews: 45,
  ...
  {
    id: 9,
    name: "Pippa the Penguin Keychain",
    cat: "Plushies",
    price: 12,
    rating: 4.7,
    reviews: 110,
    tag: "",
    img: IMG("photo-1511556820780-d912e42b4980"),
    seed: "penguin",
  },
  {
    id: 10,
    name: "Sage Nesting Baskets (Set of 3)",
    cat: "Home",
    price: 44,
    rating: 4.8,
    reviews: 52,
  ...
  {
    id: 11,
    name: "Peach Blossom Infinity Scarf",
    cat: "Wearables",
    price: 38,
    rating: 4.9,
    reviews: 69,
    tag: "New",
    img: IMG("photo-1607344645866-009c320b63e0"),
    seed: "scarf",
  },
  {
    id: 12,
    name: "Little Cloud Nursery Pillow",
    cat: "Nursery",
    price: 30,
    rating: 4.7,
    reviews: 41,
    tag: "",
    img: IMG("photo-1490481651871-ab68de25d43d"),
    seed: "pillow",
  },
];
const CATS = [
...
const TESTI = [
  {
    q: '"The baby blanket I ordered is the softest thing I have ever felt. You can tell every single stitch was made with love."',
    name: "Emma R.",
    role: "Verified buyer",
    init: "ER",
    color: "bg-blush-400",
  ...
  {
    q: '"My daughter has not put her bunny down since it arrived. The quality is unbelievable for something handmade."',
    name: "Priya S.",
    role: "Verified buyer",
    init: "PS",
    color: "bg-sage-400",
  ...
  {
    q: '"Beautiful colours, quick shipping, and the sweetest handwritten note inside the package. Ordering again for sure!"',
    name: "Dana M.",
    role: "Verified buyer",
    init: "DM",
    color: "bg-lilac-300",
...
function authTab(mode) {
  const login = $("authTabLogin"),
    signup = $("authTabSignup");
  if (mode === "login") {
    login.classList.add("bg-white", "shadow-sm", "text-blush-600");
    login.classList.remove("text-cocoa-400");
    signup.classList.remove("bg-white", "shadow-sm", "text-blush-600");
    signup.classList.add("text-cocoa-400");
    $("loginForm").classList.remove("hidden");
    $("signupForm").classList.add("hidden");
  } else {
    signup.classList.add("bg-white", "shadow-sm", "text-blush-600");
    signup.classList.remove("text-cocoa-400");
    login.classList.remove("bg-white", "shadow-sm", "text-blush-600");
    login.classList.add("text-cocoa-400");
...
function session() {
  try {
    return JSON.parse(localStorage.getItem("lb_session"));
  } catch (e) {
    return null;
  }
}
...
function doSignup(e) {
  ...
  const name = $("suName").value.trim(),
    ...
    pass = $("suPass").value;
  const err = $("suErr");
  ["suName", "suEmail", "suPass"].forEach((id) =>
  ...
  if (!/^\S+@\S+\.\S+$/.test(email)) {
    ...
    err.textContent = "That email looks a little tangled — please check it.";
    err.classList.remove("hidden");
    return;
  }
  if (pass.length < 6) {
    $("suPass").classList.add("invalid");
    err.textContent = "Password needs at least 6 characters.";
    err.classList.remove("hidden");
    return;
  }
  ...
  toast(`Welcome to the cozy club, ${name.split(" ")[0]}!`);
}
function doLogin(e) {
  e.preventDefault();
  ...
  if (!u) {
    err.textContent =
      "No account found with those details — check your email and password.";
    err.classList.remove("hidden");
    return;
  }
...
function cancelAuth() {
  ...
  const destination = pendingPage === "payment" ? "cart" : pendingPage;
...
function showPage(name) {
  ...
  // Never allowing checkout with an empty basket.
  ...
  if (!el) {
    console.warn(`Page "${name}" does not exist.`);
    return;
  }
  ...
  //Updating desktop navigation state.
  document
    .querySelectorAll("#desktopNav .nav-link")
...
function buildPills() {
  $("catPills").innerHTML = CATS.map(
    (c) =>
      `<button class="pill ${c === activeCat ? "active" : ""}" data-cat="${c}" onclick="setCat('${c}')">${c}</button>`,
  ).join("");
}
function updatePills() {
  document
    .querySelectorAll("#catPills .pill")
    .forEach((b) => b.classList.toggle("active", b.dataset.cat === activeCat));
}
function setCat(c) {
  activeCat = c;
  updatePills();
...
function renderProducts() {
  ...
  $("resultCount").textContent = list.length
    ? `${list.length} cozy ${list.length === 1 ? "piece" : "pieces"}`
    : "";
}
function renderFeatured() {
  $("featuredGrid").innerHTML = [5, 1, 11, 7]
...
function addToCart(id, btn) {
  ...
  if (btn) {
    ...
    btn.innerHTML =
      '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M20 6L9 17l-5-5"/></svg>Added';
    btn.classList.add("!bg-sage-500");
    setTimeout(() => {
      btn.innerHTML = orig;
      btn.classList.remove("!bg-sage-500");
    }, 1100);
  }
}
...
function renderCart() {
  ...
  $("cartShip").innerHTML =
    ship === 0 ? '<span class="text-sage-500">Free</span>' : money(ship);
...
function renderPaySummary() {
  ...
  $("payBtnLabel").textContent =
    (payMethod === "card" ? "Pay " : "Place order — ") + money(total);
}
...
function placeOrder(e) {
  ...
  btn.disabled = true;
  btn.innerHTML =
    '<span class="spinner"></span><span>Processing your order...</span>';
...
function completeOrder() {
  ...
  $("cfDate").textContent = new Date(Date.now() + 6 * 864e5).toLocaleDateString(
    undefined,
  ...
  $("confirmBlock").classList.remove("page-enter");
  void $("confirmBlock").offsetWidth;
  $("confirmBlock").classList.add("page-enter");
  cart = [];
  saveCart();
  updateCartBadge();
  confetti();
...
function continueShopping() {
  ...
  btn.disabled = false;
  btn.innerHTML =
    '<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="4" y="10" width="16" height="11" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg><span id="payBtnLabel">Pay $0.00 Securely</span>';
  showPage("products");
}
function confetti() {
  const colors = [
  ...
  for (let i = 0; i < 44; i++) {
    ...
    c.style.height = (Math.random() < 0.5 ? size : size * 1.5) + "px";
...
function renderTesti(i) {
  ...
  const av = $("tAvatar");
  av.className =
    "w-12 h-12 rounded-full flex items-center justify-center font-display font-bold text-white " +
    t.color;
  av.textContent = t.init;
  document
    .querySelectorAll("#tDots .dot")
...
$("tDots").innerHTML = TESTI.map(
  (_, i) =>
    `<button class="dot ${i === 0 ? "active" : ""}" onclick="gotoTesti(${i})" aria-label="Testimonial ${i + 1}"></button>`,
).join("");
setInterval(() => {
